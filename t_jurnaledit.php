...
class ct_jurnal_edit extends ct_jurnal {
	...
	function Page_Main() {
		...
		if ($this->jurnal_id->CurrentValue == "") {
			$this->Page_Terminate("t_jurnallist.php"); // Invalid key, return to list
		}
	...
	function RenderRow() {
		...
		$sSqlWrk = "SELECT `level4_id`, `level4_nama` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `t_level4`";
		$sWhereWrk = "";
		$this->akun_id->LookupFilters = array("dx1" => '`level4_nama`');
		...
		$this->nilai->ViewCustomAttributes = "";
			...
			$sSqlWrk = "SELECT `level4_id`, `level4_nama` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld`, '' AS `SelectFilterFld`, '' AS `SelectFilterFld2`, '' AS `SelectFilterFld3`, '' AS `SelectFilterFld4` FROM `t_level4`";
			$sWhereWrk = "";
			$this->akun_id->LookupFilters = array("dx1" => '`level4_nama`');
			$lookuptblfilter = "`jurnal` = 1";
			ew_AddFilter($sWhereWrk, $lookuptblfilter);
	...
	function SetupLookupFilters($fld, $pageId = null) {
		...
		case "x_akun_id":
			$sSqlWrk = "";
			$sSqlWrk = "SELECT `level4_id` AS `LinkFld`, `level4_nama` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `t_level4`";
			$sWhereWrk = "{filter}";
			$this->akun_id->LookupFilters = array("dx1" => '`level4_nama`');
			$lookuptblfilter = "`jurnal` = 1";
			ew_AddFilter($sWhereWrk, $lookuptblfilter);
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $t_jurnal->akun_id->FldCaption(), $t_jurnal->akun_id->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_jenis_jurnal");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $t_jurnal->jenis_jurnal->FldCaption(), $t_jurnal->jenis_jurnal->ReqErrMsg)) ?>");
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $t_jurnal->nilai->FldCaption(), $t_jurnal->nilai->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_nilai");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($t_jurnal->nilai->FldErrMsg()) ?>");
...
ft_jurnaledit.Lists["x_akun_id"] = {"LinkField":"x_level4_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_level4_nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t_level4"};
ft_jurnaledit.Lists["x_jenis_jurnal"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ft_jurnaledit.Lists["x_jenis_jurnal"].Options = <?php echo json_encode($t_jurnal->jenis_jurnal->Options()) ?>;
...
<?php if ($t_jurnal->akun_id->Visible) { // akun_id ?>
	<div id="r_akun_id" class="form-group">
		<label id="elh_t_jurnal_akun_id" for="x_akun_id" class="col-sm-2 control-label ewLabel"><?php echo $t_jurnal->akun_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $t_jurnal->akun_id->CellAttributes() ?>>
<span id="el_t_jurnal_akun_id">
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next().click();" tabindex="-1" class="form-control ewLookupText" id="lu_x_akun_id"><?php echo (strval($t_jurnal->akun_id->ViewValue) == "" ? $Language->Phrase("PleaseSelect") : $t_jurnal->akun_id->ViewValue); ?></span>
</span>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($t_jurnal->akun_id->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x_akun_id',m:0,n:10});" class="ewLookupBtn btn btn-default btn-sm"><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" data-table="t_jurnal" data-field="x_akun_id" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $t_jurnal->akun_id->DisplayValueSeparatorAttribute() ?>" name="x_akun_id" id="x_akun_id" value="<?php echo $t_jurnal->akun_id->CurrentValue ?>"<?php echo $t_jurnal->akun_id->EditAttributes() ?>>
<input type="hidden" name="s_x_akun_id" id="s_x_akun_id" value="<?php echo $t_jurnal->akun_id->LookupFilterQuery() ?>">
</span>
<?php echo $t_jurnal->akun_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($t_jurnal->jenis_jurnal->Visible) { // jenis_jurnal ?>
	<div id="r_jenis_jurnal" class="form-group">
		<label id="elh_t_jurnal_jenis_jurnal" for="x_jenis_jurnal" class="col-sm-2 control-label ewLabel"><?php echo $t_jurnal->jenis_jurnal->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $t_jurnal->jenis_jurnal->CellAttributes() ?>>
<span id="el_t_jurnal_jenis_jurnal">
<select data-table="t_jurnal" data-field="x_jenis_jurnal" data-value-separator="<?php echo $t_jurnal->jenis_jurnal->DisplayValueSeparatorAttribute() ?>" id="x_jenis_jurnal" name="x_jenis_jurnal"<?php echo $t_jurnal->jenis_jurnal->EditAttributes() ?>>
<?php echo $t_jurnal->jenis_jurnal->SelectOptionListHtml("x_jenis_jurnal") ?>
</select>
</span>
<?php echo $t_jurnal->jenis_jurnal->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<input type="text" data-table="t_jurnal" data-field="x_tgl" data-format="7" name="x_tgl" id="x_tgl" placeholder="<?php echo ew_HtmlEncode($t_jurnal->tgl->getPlaceHolder()) ?>" value="<?php echo $t_jurnal->tgl->EditValue ?>"<?php echo $t_jurnal->tgl->EditAttributes() ?>>
...
<script type="text/javascript">
ew_CreateCalendar("ft_jurnaledit", "x_tgl", 7);
</script>
<?php } ?>
</span>
...
<?php if ($t_jurnal->nilai->Visible) { // nilai ?>
	<div id="r_nilai" class="form-group">
		<label id="elh_t_jurnal_nilai" for="x_nilai" class="col-sm-2 control-label ewLabel"><?php echo $t_jurnal->nilai->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $t_jurnal->nilai->CellAttributes() ?>>
<span id="el_t_jurnal_nilai">
<input type="text" data-table="t_jurnal" data-field="x_nilai" name="x_nilai" id="x_nilai" size="30" placeholder="<?php echo ew_HtmlEncode($t_jurnal->nilai->getPlaceHolder()) ?>" value="<?php echo $t_jurnal->nilai->EditValue ?>"<?php echo $t_jurnal->nilai->EditAttributes() ?>>
</span>
<?php echo $t_jurnal->nilai->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>

...
$t_level2_add = NULL; // Initialize page object first

class ct_level2_add extends ct_level2 {
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("t_level2list.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "t_level2view.php")
...
// Create page object
if (!isset($t_level2_add)) $t_level2_add = new ct_level2_add();

// Page init
$t_level2_add->Page_Init();

// Page main
$t_level2_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_level2_add->Page_Render();
?>
...
var CurrentForm = ft_level2add = new ew_Form("ft_level2add", "add");

// Validate form
ft_level2add.Validate = function() {
...
ft_level2add.Form_CustomValidate = 
...
ft_level2add.ValidateRequired = true;
<?php } else { ?>
ft_level2add.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_level2add.Lists["x_level1_id"] = {"LinkField":"x_level1_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_level1_no","x_level1_nama","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t_level1"};
...
<?php } ?>
<?php $t_level2_add->ShowPageHeader(); ?>
<?php
$t_level2_add->ShowMessage();
?>
<form name="ft_level2add" id="ft_level2add" class="<?php echo $t_level2_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_level2_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_level2_add->Token ?>">
<?php } ?>
...
<script type="text/javascript">
ft_level2add.CreateAutoSuggest({"id":"x_level1_id","forceSelect":true});
</script>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_level2_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>

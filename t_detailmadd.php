...
$t_detailm_add = NULL; // Initialize page object first

class ct_detailm_add extends ct_detailm {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "t_detailmview.php")
...
// Create page object
if (!isset($t_detailm_add)) $t_detailm_add = new ct_detailm_add();

// Page init
$t_detailm_add->Page_Init();

// Page main
$t_detailm_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_detailm_add->Page_Render();
?>
...
var CurrentForm = ft_detailmadd = new ew_Form("ft_detailmadd", "add");

// Validate form
ft_detailmadd.Validate = function() {
...
ft_detailmadd.Form_CustomValidate = 
...
ft_detailmadd.ValidateRequired = true;
<?php } else { ?>
ft_detailmadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_detailmadd.Lists["x_akunm_id"] = {"LinkField":"x_level4_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_no_nama_akun","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"v_akun_jurnal"};
...
<?php } ?>
<?php $t_detailm_add->ShowPageHeader(); ?>
<?php
$t_detailm_add->ShowMessage();
?>
<form name="ft_detailmadd" id="ft_detailmadd" class="<?php echo $t_detailm_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_detailm_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_detailm_add->Token ?>">
<?php } ?>
...
<script type="text/javascript">
ft_detailmadd.CreateAutoSuggest({"id":"x_akunm_id","forceSelect":true});
</script>
...
<?php if (strval($t_detailm->jurnalm_id->getSessionValue()) <> "") { ?>
<input type="hidden" name="x_jurnalm_id" id="x_jurnalm_id" value="<?php echo ew_HtmlEncode(strval($t_detailm->jurnalm_id->getSessionValue())) ?>">
<?php } ?>
<?php if (!$t_detailm_add->IsModal) { ?>
<div class="form-group">
	<div class="col-sm-offset-2 col-sm-10">
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("AddBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_detailm_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>

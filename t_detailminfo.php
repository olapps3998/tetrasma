...
//
// Table class for t_detailm
//
class ct_detailm extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 't_detailm';
		$this->TableName = 't_detailm';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t_detailm`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// detailm_id
		$this->detailm_id = new cField('t_detailm', 't_detailm', 'x_detailm_id', 'detailm_id', '`detailm_id`', '`detailm_id`', 3, -1, FALSE, '`detailm_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// jurnalm_id
		$this->jurnalm_id = new cField('t_detailm', 't_detailm', 'x_jurnalm_id', 'jurnalm_id', '`jurnalm_id`', '`jurnalm_id`', 3, -1, FALSE, '`jurnalm_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// akunm_id
		$this->akunm_id = new cField('t_detailm', 't_detailm', 'x_akunm_id', 'akunm_id', '`akunm_id`', '`akunm_id`', 3, -1, FALSE, '`EV__akunm_id`', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'TEXT');
		...
		// nilaim_debet
		$this->nilaim_debet = new cField('t_detailm', 't_detailm', 'x_nilaim_debet', 'nilaim_debet', '`nilaim_debet`', '`nilaim_debet`', 20, -1, FALSE, '`nilaim_debet`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// nilaim_kredit
		$this->nilaim_kredit = new cField('t_detailm', 't_detailm', 'x_nilaim_kredit', 'nilaim_kredit', '`nilaim_kredit`', '`nilaim_kredit`', 20, -1, FALSE, '`nilaim_kredit`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	// Master filter
	function SqlMasterFilter_t_jurnalm() {
		return "`jurnalm_id`=@jurnalm_id@";
	}

	// Detail filter
	function SqlDetailFilter_t_jurnalm() {
		return "`jurnalm_id`=@jurnalm_id@";
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`t_detailm`";
	}
	...
	function getSqlSelectList() { // Select for List page
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT `no_nama_akun` FROM `v_akun_jurnal` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`level4_id` = `t_detailm`.`akunm_id` LIMIT 1) AS `EV__akunm_id` FROM `t_detailm`" .
			") `EW_TMP_TABLE`";
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t_detailmview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t_detailmview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "t_detailmadd.php?" . $this->UrlParm($parm);
		else
			$url = "t_detailmadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("t_detailmedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("t_detailmadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("t_detailmdelete.php", $this->UrlParm());
	}

	// Add master url
	function AddMasterUrl($url) {
		if ($this->getCurrentMasterTable() == "t_jurnalm" && strpos($url, EW_TABLE_SHOW_MASTER . "=") === FALSE) {

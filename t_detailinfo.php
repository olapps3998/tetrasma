...
class ct_detail extends cTable {
	...
	function __construct() {
		...
		// akun_id
		$this->akun_id = new cField('t_detail', 't_detail', 'x_akun_id', 'akun_id', '`akun_id`', '`akun_id`', 3, -1, FALSE, '`EV__akun_id`', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'TEXT');
		...
		// nilai
		$this->nilai = new cField('t_detail', 't_detail', 'x_nilai', 'nilai', '`nilai`', '`nilai`', 20, -1, FALSE, '`nilai`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// anggota_id
		$this->anggota_id = new cField('t_detail', 't_detail', 'x_anggota_id', 'anggota_id', '`anggota_id`', '`anggota_id`', 3, -1, FALSE, '`EV__anggota_id`', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'TEXT');
		...
		// dk
		$this->dk = new cField('t_detail', 't_detail', 'x_dk', 'dk', '`dk`', '`dk`', 16, -1, FALSE, '`dk`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlSelectList() { // Select for List page
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT `no_nama_akun` FROM `v_akun_jurnal` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`level4_id` = `t_detail`.`akun_id` LIMIT 1) AS `EV__akun_id`, (SELECT `nama` FROM `t_anggota` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`anggota_id` = `t_detail`.`anggota_id` LIMIT 1) AS `EV__anggota_id` FROM `t_detail`" .
			") `EW_TMP_TABLE`";
   ...
   // Common render codes
	...
	function Row_Inserted($rsold, &$rsnew) {
		...
		$jenis_jurnal = ew_ExecuteScalar("select jenis_jurnal from t_jurnal where jurnal_id = ".$rsnew["jurnal_id"]."");
		if ($jenis_jurnal == "M") {
			$dk = 1;
			$dk_lawan = 0;
		}
		...
		ew_Execute("update t_detail set dk = ".$dk." where jurnal_id = ".$rsnew["jurnal_id"]."");
		$total_lawan = ew_ExecuteScalar("select sum(nilai) from t_detail where jurnal_id = ".$rsnew["jurnal_id"]."");
		ew_Execute("update t_jurnal set nilai = ".$total_lawan." where jurnal_id = ".$rsnew["jurnal_id"]."");
	}
	...
	function Row_Updated($rsold, &$rsnew) {
		...
		$jenis_jurnal = ew_ExecuteScalar("select jenis_jurnal from t_jurnal where jurnal_id = ".$rsold["jurnal_id"]."");
		if ($jenis_jurnal == "M") {
			$dk = 1;
			$dk_lawan = 0;
		}
		...
		ew_Execute("update t_detail set dk = ".$dk." where jurnal_id = ".$rsold["jurnal_id"]."");
		$total_lawan = ew_ExecuteScalar("select sum(nilai) from t_detail where jurnal_id = ".$rsold["jurnal_id"]."");
		ew_Execute("update t_jurnal set nilai = ".$total_lawan." where jurnal_id = ".$rsold["jurnal_id"]."");
	}
	...
	function Row_Deleted(&$rs) {
		...
		//echo "Row Deleted";
		$row_count = ew_ExecuteScalar("select count(nilai) from t_detail where jurnal_id = ".$rs["jurnal_id"]."");
		if ($row_count > 0) {
			$total_lawan = ew_ExecuteScalar("select sum(nilai) from t_detail where jurnal_id = ".$rs["jurnal_id"]."");
			ew_Execute("update t_jurnal set nilai = ".$total_lawan." where jurnal_id = ".$rs["jurnal_id"]."");
		}
		else {
			ew_Execute("update t_jurnal set nilai = 0 where jurnal_id = ".$rs["jurnal_id"]."");
		}
	}

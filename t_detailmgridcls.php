...
$t_detailm_grid = NULL; // Initialize page object first

class ct_detailm_grid extends ct_detailm {
	...
	// Page object name
	var $PageObjName = 't_detailm_grid';

	// Grid form hidden field names
	var $FormName = 'ft_detailmgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["t_detailm"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->detailm_id->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_akunm_id") && $objForm->HasValue("o_akunm_id") && $this->akunm_id->CurrentValue <> $this->akunm_id->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nilaim_debet") && $objForm->HasValue("o_nilaim_debet") && $this->nilaim_debet->CurrentValue <> $this->nilaim_debet->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nilaim_kredit") && $objForm->HasValue("o_nilaim_kredit") && $this->nilaim_kredit->CurrentValue <> $this->nilaim_kredit->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->detailm_id->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->akunm_id->setOldValue($objForm->GetValue("o_akunm_id"));
		if (!$this->nilaim_debet->FldIsDetailKey) {
			$this->nilaim_debet->setFormValue($objForm->GetValue("x_nilaim_debet"));
		}
		$this->nilaim_debet->setOldValue($objForm->GetValue("o_nilaim_debet"));
		if (!$this->nilaim_kredit->FldIsDetailKey) {
			$this->nilaim_kredit->setFormValue($objForm->GetValue("x_nilaim_kredit"));
		}
		$this->nilaim_kredit->setOldValue($objForm->GetValue("o_nilaim_kredit"));
		if (!$this->detailm_id->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->detailm_id->CurrentValue = strval($arKeys[0]); // detailm_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		// Set up foreign key field value from Session
			if ($this->getCurrentMasterTable() == "t_jurnalm") {
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "t_jurnalm") {
			$this->jurnalm_id->Visible = FALSE;
			if ($GLOBALS["t_jurnalm"]->EventCancelled) $this->EventCancelled = TRUE;
		}

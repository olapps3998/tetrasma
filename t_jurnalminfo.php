...
//
// Table class for t_jurnalm
//
class ct_jurnalm extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 't_jurnalm';
		$this->TableName = 't_jurnalm';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t_jurnalm`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// jurnalm_id
		$this->jurnalm_id = new cField('t_jurnalm', 't_jurnalm', 'x_jurnalm_id', 'jurnalm_id', '`jurnalm_id`', '`jurnalm_id`', 3, -1, FALSE, '`jurnalm_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// no_buktim
		$this->no_buktim = new cField('t_jurnalm', 't_jurnalm', 'x_no_buktim', 'no_buktim', '`no_buktim`', '`no_buktim`', 200, -1, FALSE, '`no_buktim`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->no_buktim->Sortable = TRUE; // Allow sort
		$this->fields['no_buktim'] = &$this->no_buktim;

		// tglm
		$this->tglm = new cField('t_jurnalm', 't_jurnalm', 'x_tglm', 'tglm', '`tglm`', ew_CastDateFieldForLike('`tglm`', 7, "DB"), 133, 7, FALSE, '`tglm`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// ketm
		$this->ketm = new cField('t_jurnalm', 't_jurnalm', 'x_ketm', 'ketm', '`ketm`', '`ketm`', 201, -1, FALSE, '`ketm`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
	...
	function GetDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "t_detailm") {
			$sDetailUrl = $GLOBALS["t_detailm"]->GetListUrl() . "?" . EW_TABLE_SHOW_MASTER . "=" . $this->TableVar;
			$sDetailUrl .= "&fk_jurnalm_id=" . urlencode($this->jurnalm_id->CurrentValue);
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "t_jurnalmlist.php";
		}
		return $sDetailUrl;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`t_jurnalm`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$rs['jurnalm_id'] = $this->jurnalm_id->DbValue;
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('jurnalm_id', $rs))
				ew_AddFilter($where, ew_QuotedName('jurnalm_id', $this->DBID) . '=' . ew_QuotedValue($rs['jurnalm_id'], $this->jurnalm_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->jurnalm_id->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@jurnalm_id@", ew_AdjustSql($this->jurnalm_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t_jurnalmview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t_jurnalmview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "t_jurnalmadd.php?" . $this->UrlParm($parm);
		else
			$url = "t_jurnalmadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t_jurnalmedit.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t_jurnalmedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t_jurnalmadd.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t_jurnalmadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("t_jurnalmdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "jurnalm_id:" . ew_VarToJson($this->jurnalm_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->jurnalm_id->CurrentValue)) {
			$sUrl .= "jurnalm_id=" . urlencode($this->jurnalm_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["jurnalm_id"]))
				$arKeys[] = ew_StripSlashes($_POST["jurnalm_id"]);
			elseif (isset($_GET["jurnalm_id"]))
				$arKeys[] = ew_StripSlashes($_GET["jurnalm_id"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->jurnalm_id->CurrentValue = $key;
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->tglm->EditValue = ew_FormatDateTime($this->tglm->CurrentValue, 7);
